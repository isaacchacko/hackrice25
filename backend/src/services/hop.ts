...
import { search, type Page } from './search.js';
...
const hopSessions = new Map<string, HopState>();

export async function createHopSession(query: string, sessionId: string): Promise<HopState> {
...
export function getHopSession(sessionId: string): HopState | null {
...
export function navigateHop(sessionId: string, direction: 'next' | 'prev'): HopState | null {
  const session = hopSessions.get(sessionId);
  if (!session) {
...
export function getCurrentPage(sessionId: string): Page | null {
  const session = hopSessions.get(sessionId);
  if (!session) {
...
export function getAllPages(sessionId: string): Page[] | null {
  const session = hopSessions.get(sessionId);
  if (!session) {
...
export function deleteHopSession(sessionId: string): boolean {
